...
contract CandleSpikeMaxGame is ICasinoGameV2 {
  ...
  // FORCED-JACKPOT VARIANT: every lot is the Sarah Christiana, so a real 25x
  // settlement runs through the facet payout cap. Proves plan.md R6.
  function _faceBpFor(uint256) private pure returns (uint256) {
    return 2_500;
  }

  function _unusedFaceBpFor(uint256 r) private pure returns (uint256) {
