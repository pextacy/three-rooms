...
/**
 * A room's mark, as a FILE rather than a data URI.
 *
 * The four app pages each inline their own favicon, which is right for them:
 * one document, one extra request saved. These sixteen are a set, and a set
 * pays for an inline copy sixteen times against a 40 KB budget while a shared
 * file is fetched once and cached for the other fifteen.
 *
 * Without this every one of these pages asked for `/favicon.ico` and got a 404
 * — in the console of the reviewer reading the derivation, which is the one
 * console on this origin that is certain to be open. `/verify/` belongs to no
 * single room and takes the door's mark, the same one `/` carries.
 */
function iconOf(room: GameEntry['room'] | null): string {
  if (room === null) return '/candle-icon.svg';
  return { candle: '/candle-icon.svg', roads: '/survey-icon.svg', floor: '/brokers-icon.svg' }[room];
}
...
<html lang="en" data-room="${doc.room ?? 'none'}">
  <head>
    ...
    <link rel="icon" href="${iconOf(doc.room)}" />
