/**
 * `npm run deploy:contract` — deploys Candle.sol to a real chain.
 *
...
if (!rpc || !key) {
  ...
  console.error(D('  RPC_URL=https://… DEPLOYER_KEY=0x… npm run deploy:contract\n'));
...
const artifact = JSON.parse(readFileSync(join(ROOT, 'contracts/out/Candle.sol/CandleGame.json'), 'utf8'));
...
const out = execFileSync(
  'forge',
  ['create', 'contracts/Candle.sol:CandleGame', '--rpc-url', rpc, '--private-key', key, '--broadcast'],
...
check('maxReservedProfit is 24x the wager', maxReserved === wager * 24n, 'no slack on the 25x cap');
check('the facet cap equals the maximum payout', maxEscrow + maxReserved === wager * 25n);
...
check('maxPayout is 25x and agrees with quoteCaps', maxPayout === wager * 25n && maxPayout === maxEscrow + maxReserved);
check('probabilityWad is the top tier only', probabilityWad === 2_000_000_000_000_000n, '0.20%');
check('expectedPayout is the declared RTP', expectedPayout === (wager * 7_577_820_426_157n) / 7_812_500_000_000n, '96.9961%');
check('subJackpotVarianceScaled is 0 — CANDLE is not heavy-tailed', subVariance === 0n);
